...
### 산문 우선
설명을 보강할 때 불릿 리스트가 아닌 산문(문단)으로 풀어 씁니다. 불릿은 병렬 비교, 요약, 체크리스트에만 사용합니다.
...
# After
DNS 서버가 질의를 받으면, 돌려줘야 할 정보의 종류가 다양합니다.
...
# After
모든 이름을 하나의 파일에 넣는 방식이 먼저 시도되었습니다.
...
초보자 독자를 위해서는 암묵적 전제를 예상보다 더 깊이 풀어야 합니다.

```
# Before
컨테이너의 패킷이 docker0 브릿지를 거쳐 다른 호스트로 전달됩니다.
→ docker0가 무엇인지, 왜 다른 호스트로 직접 전달되지 않는지 설명 없음

# After
컨테이너의 veth 인터페이스에서 출발한 패킷은 docker0 브릿지에 도착합니다.
docker0는 호스트 내부의 가상 L2 스위치로, 같은 브릿지에 연결된 컨테이너 사이에서
ARP를 통해 IP → MAC 주소를 알아내고, MAC 주소 기반으로 프레임을 전달합니다.

하지만 docker0 브릿지는 호스트 내부의 가상 네트워크이므로,
ARP 요청은 물리 네트워크로 나가지 않습니다.
다른 호스트의 컨테이너에 패킷을 보내려면 별도의 메커니즘이 필요합니다.
→ ARP, MAC 주소, docker0와 물리 네트워크의 구분 등 숨겨진 전제가 모두 명시됨
```
...
**중요**: 확립된 기술 용어(L2, L3, ARP, MAC, TTL 등)는 제거하거나 단순화하지 않습니다. 대신 설명을 추가합니다. 친숙하지만 범위가 다른 용어로 치환하는 것도 금지합니다 (예: "L2" → "이더넷"은 L2 ⊃ 이더넷이므로 정확성 훼손).

1. **즉시 설명 추가**: 핵심 개념이면 괄호나 후속 문장으로 설명
2. **비유/예시로 설명**: 추상적 개념이면 익숙한 것에 비유
3. **삭제 또는 대체**: 글의 흐름에 필수적이지 않고, 확립된 기술 용어가 아닌 경우에만 일반적인 표현으로 대체

**금지 패턴**: 이해도를 높인다는 이유로 정밀한 용어를 느슨한 용어로 바꾸거나, 구체적 내용을 생략하지 않습니다. 정확한 용어를 유지하고 설명을 추가하는 것이 올바른 접근입니다.
