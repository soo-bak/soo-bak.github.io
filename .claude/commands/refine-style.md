...
### 대상 독자
**초보자**가 외부 자료 없이 이 글만으로 이해할 수 있는 수준을 기준으로 합니다. 서사적 도입과 톤 조정 시에도 초보자의 이해를 해치지 않아야 합니다.

### 산문 우선
설명이 필요한 항목은 불릿 리스트가 아닌 산문(문단)으로 풀어 씁니다. 불릿은 병렬 비교, 요약, 체크리스트에만 사용합니다. 서사적 도입, 섹션 간 전환, 독자 참여 유도 모두 산문으로 작성합니다.
...
같은 어미가 연속되지 않도록 자연스럽게 변화를 줍니다. 기계적으로 어미를 교체하는 것이 아니라, 내용의 흐름에 맞는 어미를 선택합니다.
...
1. **이유 제시로 전환**: "~합니다" 연속 → 중간에 "~때문입니다" 또는 "~있습니다" 삽입
2. **절 연결로 합치기**: 짧은 "~입니다" 두 개 → "~이며, ~입니다"
3. **능동/수동 전환**: "관리됩니다" → "관리합니다" (또는 역방향)
4. **선언으로 전환**: "~합니다" 연속 → 중간에 "하지만 문제가 있습니다" 같은 선언문 삽입
...
### 점검 항목
- [ ] 섹션이 "A는 B이다" 정의로 시작하는가? → 문제/배경으로 전환
...
3. "그런데" → 선언문("하지만 문제가 있습니다")으로, "따라서" → 결과를 직접 서술
...
|------|----------------------|--------------|
| 개념 도입 | "A는 B이다" | 문제 제시 → "그래서 A가 등장했다" |
| 독자 참여 | 없음 | 문제 제시로 유도: "하지만 3번이 아니면 문제가 생깁니다." |
...
# After
웹 서버에 1000명이 동시에 접속하면, 커널은 1000개의 연결을 구분해야 합니다.
이때 사용되는 것이 5-tuple입니다.
```
...
| "중요한 점은 ~입니다" | 바로 내용 서술 |
| "~라고 할 수 있습니다" | "~입니다" |
| "그런데 여기서 근본적인 질문이 생깁니다" | "하지만 문제가 있습니다" |
...
- [ ] 독자가 "왜?"를 느낄 수 있는 문제 제시가 적절히 배치되어 있는가?
...
| 문제 제기로 전환 | "하지만 이것만으로는 ~를 해결할 수 없습니다." |
...
굵은 글씨는 네 가지 용도로만 사용합니다: 핵심 선언, 핵심 철학/원칙, 문제 선언, 하위 주제 도입.
...
**문제 선언 (독립 줄)**
```markdown
**다른 컴퓨터에 있는 프로세스와 통신하려면 새로운 추상화가 필요했습니다.**
```
...
- 굵은 글씨 정비
- 흐름과 맥락 연결 확인
- **주의**: 문체 전환 과정에서 암묵적 전제(중간 단계 생략)를 도입하지 않습니다. 문장을 합치거나 재구성할 때 원래 있던 인과 관계의 중간 단계가 빠지지 않았는지 확인합니다.
...
- [ ] 독자 참여(문제 제시, 오해-교정)가 적절히 배치되어 있는가?
- [ ] 질문형 문체가 사용되지 않았는가?
- [ ] 앞에서 설명한 개념이 뒤에서 활용될 때 명시적으로 연결되는가?

### 글 전체 수준
- [ ] 백과사전형이 아닌 멘토형 톤이 유지되는가?
- [ ] 굵은 글씨가 네 가지 용도(선언, 원칙, 문제 선언, 하위 주제)로만 사용되는가?
