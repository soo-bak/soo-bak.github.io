...
좌표 압축은 좌표 값을 상대적 순위로 변환하는 기법입니다. 입력 배열을 복사하여 정렬하고 중복을 제거하면, 서로 다른 좌표들이 오름차순으로 정렬된 배열을 얻을 수 있습니다. 이 배열에서 각 좌표의 인덱스가 해당 좌표보다 작은 서로 다른 좌표의 개수가 됩니다.

원본 배열의 각 좌표를 정렬된 배열에서 찾아 그 위치로 변환합니다. C#에서는 딕셔너리에 미리 인덱스를 저장해 두어 `O(1)` 조회를 하고, C++에서는 이진 탐색 `lower_bound`를 사용하여 `O(log n)` 조회를 합니다.
