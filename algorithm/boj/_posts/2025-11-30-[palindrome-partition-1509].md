...
길이 1은 항상 팰린드롬입니다. 길이 2는 두 문자가 같으면 팰린드롬입니다. 길이 3 이상은 양 끝 문자가 같고(`s[i] == s[j]`) 안쪽이 팰린드롬(`pal[i+1][j-1]`)이면 팰린드롬입니다. 길이 순서대로 계산하면 안쪽 부분이 항상 먼저 계산되어 있으므로, O(N²) 시간에 모든 부분 문자열을 판정할 수 있습니다.
