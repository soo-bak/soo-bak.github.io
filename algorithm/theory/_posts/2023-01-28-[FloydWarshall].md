---
layout: single
title: "플로이드-워셜 알고리듬 (Floyd-Warshall algorithm) - soo:bak"
date: "2023-01-28 04:49:00 +0900"
description: 플로이드-워셜 알고리듬의 개념과 원리를 설명합니다. 모든 노드 쌍 간의 최단 경로를 O(n³) 시간 복잡도로 구하는 방법을 다룹니다.
---

## 개념
  - [벨만-포드 알고리듬](https://soo-bak.github.io/algorithm/theory/BellmanFord/#page-title), [다익스트라 알고리듬](https://soo-bak.github.io/algorithm/theory/Dijkstra/#page-title)과 다르게, <b><u>한 번의 실행</u></b>으로 모든 노드들 간의 최단 경로를 구할 수 있는 알고리듬
  - 이 알고리듬에서는 노드들 간의 거리를 저장하기 위해 `행렬` 을 사용하며, 행렬의 초깃값은 `그래프의 인접 행렬`의 값과 같음<br>
  - 알고리듬은 여러 라운드로 구성
    - 각 라운드마다 `중간 노드` 로 사용할 노드를 선택하여, 해당 노드와 인접하는 노드들을 잇는 연결 거리값을 구하여 행렬에 저장 <br>
<br><br>

## 특징
  - 플로이드-워셜 알고리듬은 `3중으로 중첩된 반복문` 으로 구성되어 있고, 각각은 그래프의 노드 수 만큼 반복을 하므로, 시간 복잡도는 &nbsp;**O(n<sup>3</sup>)**
  - 구현이 간단하기 때문에, 그래프에서 한 쌍의 노드 사이의 최단 경로만 구하려 할 때에도 이 알고리듬을 사용할 수 있음
    - 단, 그래프의 크기가 작아서 세 제곱 시간 알고리듬으로도 해결이 가능한 경우에만 가능
<br><br><br>
...
## 설명
  ...
  *예시 그래프*
  ...
  - 알고리듬이 종료되고 나면, 행렬에는 모든 노드들 간의 최단 거리가 저장되게 됨<br>
