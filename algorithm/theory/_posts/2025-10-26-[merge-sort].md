...
모든 경우에 $$O(n \log n)$$의 시간 복잡도를 보장하며, 같은 값을 가진 원소들의 상대적 순서를 유지하는 **안정 정렬(Stable Sort)**이라는 특징이 있습니다.
...
입력 데이터의 초기 상태와 무관하게 항상 동일한 시간 복잡도를 가집니다.
...
배열의 크기가 충분히 작을 때는 재귀 호출의 오버헤드가 오히려 비효율적입니다. 

이런 경우 단순한 삽입 정렬을 사용하면 성능을 개선할 수 있습니다.
...
이미 정렬된 두 부분 배열의 경우, 왼쪽 부분의 마지막 원소가 오른쪽 부분의 첫 원소보다 작거나 같으면 병합 작업을 생략할 수 있습니다.

이 최적화는 거의 정렬된 데이터에서 특히 효과적입니다.
