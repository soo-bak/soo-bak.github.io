...
**피벗(pivot)** 을 기준으로 배열을 두 부분으로 나눕니다. 피벗보다 작은 원소는 왼쪽, 큰 원소는 오른쪽으로 보냅니다. 분할 후에는 피벗이 정확히 자기 위치에 있게 되므로, 결합 단계가 필요 없습니다.
...
분할정복을 이해하면 많은 효율적인 알고리즘의 원리를 파악할 수 있습니다. 재귀 구조를 이해하고, 분할-정복-결합의 세 단계를 명확히 정의하는 것이 핵심입니다.
