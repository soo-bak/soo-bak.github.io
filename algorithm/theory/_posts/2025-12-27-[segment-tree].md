...
길이가 10만인 배열이 있고, "3번째부터 7만번째까지의 합은?"이라는 질문에 답해야 한다고 가정해보겠습니다. 단순히 반복문으로 더하면 O(n)이 걸립니다. 이런 질문이 10만 번 들어오면 총 O(n²) = 100억 번의 연산이 필요합니다.

<br>

**세그먼트 트리(Segment Tree)** 는 이런 **구간 쿼리**를 **O(log n)** 에 처리하는 자료구조입니다. 구간 합, 구간 최솟값, 구간 최댓값 등 다양한 구간 연산을 빠르게 수행할 수 있습니다.
...
누적 합(Prefix Sum)은 구간 합을 O(1)에 구할 수 있지만, 값이 바뀌면 누적 합 배열 전체를 다시 계산해야 합니다. **쿼리와 업데이트가 모두 빈번**할 때 세그먼트 트리가 유용합니다.
...
각 노드는 특정 **구간**을 담당합니다. 루트는 전체 구간을 담당하고, 왼쪽 자식은 왼쪽 절반, 오른쪽 자식은 오른쪽 절반을 담당합니다.
...
지금까지는 **점 업데이트**(하나의 원소만 변경)를 다뤘습니다. 만약 **구간 업데이트**(여러 원소를 한 번에 변경)가 필요하다면?

<br>

구간 `[l, r]`의 모든 값에 `val`을 더하는 경우, 각 원소를 개별 업데이트하면 O((r-l+1) × log n)이 걸립니다. **Lazy Propagation**을 사용하면 O(log n)에 가능합니다.

<br>

### 아이디어

업데이트를 **즉시 반영하지 않고**, **나중에 필요할 때** 자식에게 전파합니다. 각 노드에 **지연된 업데이트 값(lazy)** 을 저장합니다.
...
세그먼트 트리는 구간 쿼리와 업데이트를 O(log n)에 처리하는 강력한 자료구조입니다.
