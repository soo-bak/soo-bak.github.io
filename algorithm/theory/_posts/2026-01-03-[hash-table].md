...
충돌이 발생하면 **다른 빈 슬롯**을 찾아 저장하는 방식입니다.
...
리사이징 시 모든 원소를 새로운 테이블에 다시 삽입해야 하므로 $$O(n)$$의 시간이 소요됩니다.

하지만 리사이징은 드물게 발생하므로, 분할 상환 분석(Amortized Analysis)으로 보면 여전히 평균 $$O(1)$$입니다.
