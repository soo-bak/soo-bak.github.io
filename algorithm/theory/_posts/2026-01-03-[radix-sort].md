...
[계수 정렬](https://soo-bak.github.io/algorithm/theory/counting-sort/)을 서브루틴으로 사용하여, **$$O(d \cdot (n + k))$$** 시간에 정렬합니다.
...
배열 `[170, 45, 75, 90, 802, 24, 2, 66]`을 정렬하는 예시:
...
| 항목 | 복잡도 | 설명 |
|------|--------|------|
| 시간 | $$O(d \cdot (n + k))$$ | $$d$$: 자릿수, $$k$$: 기수 |
| 공간 | $$O(n + k)$$ | 출력 배열 + 계수 배열 |
...
| 특성 | 기수 정렬 | 빠른 정렬 |
|------|----------|----------|
| 시간 복잡도 | $$O(d \cdot n)$$ | $$O(n \log n)$$ |
| 공간 복잡도 | $$O(n + k)$$ | $$O(\log n)$$ |
| 안정성 | 안정 (LSD) | 불안정 |
| 데이터 타입 | 정수 | 모든 비교 가능 타입 |

<br>

기수 정렬은 자릿수 $$d$$가 $$\log n$$보다 작을 때 빠른 정렬보다 효율적입니다.
...
YYYYMMDD 형식의 날짜를 빠르게 정렬할 수 있습니다.

<br>

## 마무리

기수 정렬은 자릿수별로 정렬을 반복하여 선형 시간에 정렬하는 알고리듬입니다.

<br>

**핵심 포인트**
- **자릿수별 정렬**: 낮은 자리부터 (LSD) 또는 높은 자리부터 (MSD)
- **서브루틴**: 계수 정렬을 각 자릿수에 적용
- **시간 복잡도**: $$O(d \cdot (n + k))$$
- **적용 조건**: 정수, 자릿수가 적은 경우
