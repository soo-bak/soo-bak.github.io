...
$(function () {
  $(".nav__sub-title-dropdownicon").click(function () {
    ...
    if (numItems > 0) {
      // children이 있을때만 (+/-) 아이콘 표시를 위한 <span> 태그 하나 더 생김
...
function resizeSidebarFont() {
  let navListWidth =
    document.getElementsByClassName("nav__list")[0].clientWidth;
  let resizeCnt = 10;
  var widthLimit;
  ...
  $("a[id^=sidebar-] .title__name").each(function (idx, titleName) {
    ...
    if ($titleLink.attr("titledepth") == 0) {
      ...
      widthLimit =
        navListWidth - $statIcon.outerWidth(true) - $dropdownIcon.outerWidth();
    } else {
