...
1991년 팀 버너스리(Tim Berners-Lee)가 월드 와이드 웹과 함께 HTTP를 설계했습니다.

단순한 문서 전송을 위한 프로토콜이었습니다.

<br>

30년이 지난 지금, 웹은 복잡한 애플리케이션 플랫폼이 되었습니다.

HTTP도 함께 진화해왔습니다.

어떤 문제들을 어떻게 해결해왔는지 살펴봅니다.
...
웹페이지 하나에 HTML, CSS, JavaScript, 이미지 수십 개가 필요합니다.

각 리소스마다 TCP 연결을 새로 맺어야 합니다.
...
하나의 TCP 연결을 여러 요청에 재사용합니다.

HTTP/1.1에서 기본 동작입니다.

연결 설정 오버헤드가 크게 줄었습니다.
...
요청 1의 응답이 늦으면, 요청 2, 3의 응답도 대기해야 합니다.
...
각 도메인에 6개씩 연결하면 총 24개 연결.

하지만 DNS 조회, 연결 설정 비용이 추가됩니다.
...
2015년 RFC 7540으로 표준화되었습니다.

Google의 SPDY 프로토콜에서 발전했습니다.
...
각 스트림의 프레임이 인터리빙(interleaving)되어 전송됩니다.

응답 순서와 상관없이 완료된 것부터 전달됩니다.
...
":method: GET"을 보내는 대신 "2"만 전송합니다.

<br>

**동적 테이블**

연결 중에 새 헤더가 나오면 동적 테이블에 추가합니다.

이후 같은 헤더는 인덱스로 참조합니다.
...
테이블에 없는 문자열은 허프만 코딩으로 압축합니다.

자주 사용되는 문자에 짧은 코드를 할당합니다.

<br>

결과: 헤더 크기가 85~90% 감소합니다.
...
결과적으로 서버 푸시는 잘 사용되지 않습니다.

HTTP/3에서도 지원은 하지만 권장하지 않습니다.

---

## HTTP/2의 HOL Blocking 문제

HTTP/2는 HTTP 레벨의 HOL Blocking을 해결했습니다.

하나의 스트림이 느려도 다른 스트림은 진행됩니다.
...
여러 스트림이 하나의 TCP 연결을 공유합니다.

패킷 3이 스트림 1의 것이라도, 스트림 2, 3도 모두 대기해야 합니다.
...
애플리케이션 코드는 대부분 수정 없이 동작합니다.

서버/클라이언트 라이브러리가 전송 계층을 처리합니다.
