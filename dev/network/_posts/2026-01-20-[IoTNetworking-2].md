...
[Part 1](/dev/network/IoTNetworking-1/)에서 MQTT, CoAP, BLE, Zigbee 등 IoT 프로토콜을 살펴보았습니다.

<br>

이 프로토콜들은 근거리 통신(수십~수백 미터)에 특화되어 있습니다. 그렇다면 농장이나 도시 인프라처럼 수 킬로미터 떨어진 센서와는 어떻게 통신할까요?

<br>

또한 수십억 개의 디바이스가 인터넷에 연결되면서 보안 문제도 심각해지고 있습니다. 컴퓨팅 자원이 부족한 디바이스를 어떻게 보호할 수 있을까요?
...
LPWAN은 높은 데이터 전송 속도를 포기하는 대신, 커버리지와 전력 효율에 집중합니다. 예를 들어, 농업 센서가 하루에 몇 번 온도와 습도를 전송하는 용도라면 초당 수 Mbps의 대역폭은 불필요합니다. 이런 시나리오에서 LPWAN은 단 몇 바이트의 데이터를 수 킬로미터 떨어진 게이트웨이에 도달시키는 데 최적화되어 있습니다.
...
CSS는 신호를 넓은 주파수 대역에 분산시켜 전송하기 때문에, 특정 주파수의 간섭이나 노이즈가 있어도 전체 신호를 복원할 수 있습니다. 이 방식이 LoRa가 도시 환경의 복잡한 전파 간섭 속에서도 수 킬로미터 통신을 가능하게 하는 핵심 원리입니다.
...
LoRaWAN과 NB-IoT의 선택은 인프라 환경과 운영 모델에 따라 결정됩니다. 자체 게이트웨이를 설치할 수 있고 월 구독료 없이 운영하고 싶다면 LoRaWAN이 유리하고, 이미 통신사 커버리지가 확보된 지역에서 빠르게 서비스를 시작해야 한다면 NB-IoT가 현실적입니다.
...
엣지에서 데이터를 처리하면 클라우드 왕복에 따른 지연을 제거하고, 원시 데이터 대신 가공된 결과만 전송하여 대역폭을 크게 절약할 수 있습니다. 특히 공장 자동화처럼 밀리초 단위 응답이 필요하거나, 의료 영상처럼 민감한 데이터를 외부로 보내기 어려운 환경에서 엣지 컴퓨팅은 필수적입니다.
