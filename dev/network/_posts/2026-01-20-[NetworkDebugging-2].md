...
이 도구들은 "연결이 되는가", "경로가 어디서 끊기는가"와 같은 큰 질문에 답할 수 있습니다. 하지만 "TCP 핸드셰이크가 왜 실패하는가", "TLS 인증서 교환에서 무엇이 잘못되었는가"처럼 프로토콜 내부의 동작을 확인해야 할 때는 더 깊은 도구가 필요합니다.

<br>

**패킷 캡처**는 네트워크를 지나는 데이터를 바이트 단위로 직접 확인할 수 있게 해줍니다.
...
TCP 플래그는 패킷의 목적을 나타내며, tcpdump 출력에서 대괄호 안에 약어로 표시됩니다. 정상적인 연결 수립은 `[S]` -> `[S.]` -> `[.]` 순서로 나타나고, 정상 종료는 `[F.]`로 시작합니다. `[R]`이나 `[R.]`이 보이면 비정상 종료를 의미하므로 원인 조사가 필요합니다.
...
tcpdump가 텍스트 기반의 빠른 캡처에 적합하다면, Wireshark는 캡처된 데이터의 심층 분석에 강점이 있습니다. 패킷의 각 계층을 트리 구조로 펼쳐볼 수 있고, 수백 개의 프로토콜을 자동으로 해석하여 사람이 읽기 쉬운 형태로 보여줍니다.
...
Wireshark는 두 종류의 필터를 사용하며, 적용 시점이 다릅니다. 캡처 필터는 저장 자체를 제한하므로 디스크와 메모리를 절약할 수 있지만, 한 번 놓친 패킷은 복구할 수 없습니다. 디스플레이 필터는 이미 캡처된 데이터 안에서 조건에 맞는 패킷만 화면에 표시하므로, 분석 시점에 자유롭게 조건을 변경할 수 있습니다.
...
RST의 원인을 파악하려면 RST 패킷이 발생한 시점의 전후 맥락을 살펴야 합니다. 연결 초기(SYN 직후)에 RST가 오면 포트나 방화벽 문제이고, 데이터 전송 중에 RST가 오면 애플리케이션이나 중간 장비의 문제를 의심합니다. RST의 출발지 IP를 확인하면 어느 장비가 연결을 끊었는지 알 수 있습니다.
...
SYN을 보냈는데 아무 응답이 없다는 것은 패킷이 목적지까지 도달하지 못했거나, 도달했지만 응답이 돌아오지 못한 것입니다. 방화벽이 DROP 정책을 사용하면 RST조차 보내지 않으므로 타임아웃으로 나타납니다.
