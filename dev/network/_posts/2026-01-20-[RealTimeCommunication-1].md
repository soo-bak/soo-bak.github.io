...
화상 통화, 음성 통화, 라이브 스트리밍과 같은 실시간 애플리케이션은 웹 페이지 로딩이나 파일 다운로드와는 다른 요구사항을 갖습니다.
...
Jitter가 크면 수신 측 버퍼에 패킷이 너무 빨리 쌓이거나 비어버려, 재생이 끊기거나 품질이 저하됩니다.

### 손실 허용 (Loss Tolerance)

실시간 미디어에서는 일부 패킷 손실을 감수합니다.

TCP는 패킷 하나라도 빠지면 재전송으로 복구합니다. 그러나 음성 통화에서 20ms짜리 음성 조각 하나가 빠져도 사람의 귀는 거의 알아채지 못합니다. 영상에서도 한두 프레임이 빠지면 잠깐 화면이 흐려질 뿐, 재전송을 기다리며 전체가 멈추는 것보다 낫습니다.
...
TCP의 혼잡 제어는 패킷 손실을 네트워크 혼잡의 신호로 해석하고, 전송률을 절반으로 줄입니다. 일반 데이터 전송에서는 합리적인 전략이지만, 실시간 미디어는 일정한 비트레이트를 유지해야 자연스러운 재생이 가능합니다.

<br>

그런데 일시적 손실에도 전송률이 절반으로 줄고, 회복까지 수 초가 걸립니다. 영상 통화 중 해상도가 갑자기 떨어졌다 올라갔다 하는 현상이 여기서 비롯됩니다.

---

## RTP (Real-time Transport Protocol)

TCP의 재전송, 순서 보장, 혼잡 제어가 실시간 미디어에는 오히려 방해가 된다면, 어떤 프로토콜을 써야 할까요?
...
즉, RTP는 **프레임워크**로서 미디어 전송에 필요한 정보를 제공하되, 손실과 Jitter에 대응하는 방법은 애플리케이션이 결정합니다.
...
차이점은 RTP/UDP/IP는 재전송·순서 보장·혼잡 제어 없이, 실시간 재생에 필요한 타이밍과 순서 정보만 담고 있다는 점입니다.
...
초기값은 무작위로 설정됩니다. 고정된 값(예: 0)으로 시작하면 공격자가 시퀀스 번호를 예측하여 위조 패킷을 삽입할 수 있기 때문입니다.

### Timestamp (32비트)

미디어가 **샘플링된 시점**을 나타냅니다.
...
타임스탬프는 패킷이 네트워크에 도착한 시간과는 무관합니다. "이 미디어가 언제 생성되었는가"를 알려주므로, 수신 측은 네트워크 지연과 무관하게 원래의 재생 간격을 복원할 수 있습니다. 화상 회의에서 오디오와 비디오의 입술 싱크가 맞는 것도 이 타임스탬프 덕분입니다.
...
**Synchronization Source** 식별자로, 각 미디어 스트림을 고유하게 식별합니다. 중앙 서버가 번호를 배정하지 않으므로, 각 참가자가 무작위로 32비트 값을 생성하여 충돌 가능성을 최소화합니다.
...
RTP는 미디어를 전송하지만, 상대방이 잘 받고 있는지 알 방법이 없습니다. 패킷이 손실되고 있는지, Jitter가 얼마나 심한지, 비트레이트를 줄여야 하는지 — 이런 피드백 없이는 품질 관리가 불가능합니다.

<br>

**RTCP**는 이 문제를 해결하는 RTP의 제어 프로토콜로, 품질 피드백과 참가자 정보를 교환하며, 세션의 시작과 종료를 관리합니다. 일반적으로 RTP 포트에 1을 더한 포트를 사용합니다 (예: RTP 5004, RTCP 5005).
...
SDES는 SSRC만으로는 알 수 없는 참가자의 부가 정보를 제공합니다. 특히 CNAME(Canonical Name)은 동일한 소스를 일관되게 식별하기 위한 필수 항목입니다.
...
세션 종료를 상대방에게 알리는 패킷입니다. 참가자가 통화에서 나가면 BYE 패킷을 전송하여, 다른 참가자들이 해당 SSRC의 스트림이 의도적으로 종료되었음을 알 수 있게 합니다. BYE 없이 갑자기 스트림이 끊기면, 수신 측은 네트워크 장애인지 퇴장인지 구분할 수 없습니다.

#### APP

애플리케이션 고유의 데이터를 전송하는 데 사용됩니다. 표준 RTCP 패킷 유형으로는 표현할 수 없는 애플리케이션별 제어 정보(예: 화면 공유 상태 변경, 커스텀 품질 지표)를 교환할 때 활용됩니다.

---

## RTT 측정

SR과 RR에 담긴 타임스탬프 정보를 조합하면, 두 참가자 사이의 **왕복 시간(RTT)**도 측정할 수 있습니다.
...
송신자 A가 SR을 보낸 시각(LSR)을 수신자 B가 기억해 두었다가, RR에 LSR과 자신의 처리 시간(DLSR)을 담아 돌려보냅니다. A는 RR을 받은 현재 시각(T2)에서 LSR과 DLSR을 빼면 순수 네트워크 왕복 시간을 알 수 있습니다. 이 RTT 값은 적응형 비트레이트 조절과 Jitter Buffer 크기 결정에 직접 활용됩니다.
...
RTP가 "어떻게 보낼 것인가"의 틀을 제공한다면, "무엇을 보정하고, 어떻게 적응할 것인가"는 그 위에 쌓아 올려야 할 몫입니다.

<br>

그렇다면 브라우저에서 별도 플러그인 없이 RTP 기반 실시간 통신을 하려면 어떤 구조가 필요할까요? 시그널링, NAT 통과, 암호화를 브라우저가 어떻게 처리하는지 궁금해집니다.
