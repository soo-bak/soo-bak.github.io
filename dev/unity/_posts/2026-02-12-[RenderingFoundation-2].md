...
Part 1에서 메쉬가 3D 형태를 정의했다면, 텍스처는 그 표면에 색과 질감을 입힙니다. 하지만 같은 메쉬와 텍스처라도 금속처럼 빛을 반사하거나 천처럼 흡수하는 등 표면의 반응이 다를 수 있습니다. 이 렌더링 과정을 GPU에서 수행하는 프로그램이 **셰이더(Shader)**입니다. Part 1에서 등장한 버텍스 셰이더는 정점의 위치를 화면 좌표로 변환하고, **프래그먼트 셰이더**는 각 픽셀의 최종 색상을 계산합니다. 이 글에서 다룬 텍스처는 프래그먼트 셰이더가 픽셀 색상을 결정할 때 참조하는 데이터입니다.
