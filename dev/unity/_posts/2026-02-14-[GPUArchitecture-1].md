...
렌더링은 정점 10만 개 각각에 **같은** 좌표 변환을, 픽셀 200만 개 각각에 **같은** 색상 계산을 적용해야 하는 작업이므로, 데이터만 다를 뿐 연산은 동일합니다. 렌더링을 GPU에서 실행하는 것도 이 때문입니다.
...
다만, 32개 스레드가 **모두 같은 방향으로 분기하면** 이 비용은 발생하지 않습니다. 전부 true라면 true 경로만, 전부 false라면 false 경로만 실행하면 되기 때문입니다. 비용이 발생하는 것은 **같은 Warp 안에서** 분기 방향이 달라지는 경우뿐입니다.
...
- CPU는 레이턴시 중심, GPU는 스루풋 중심으로 설계되었습니다. GPU는 SIMT 모델에 따라 같은 셰이더를 Warp(32스레드) 단위로 묶어 병렬 실행하며, Warp 내에서 분기 방향이 달라지면 양쪽 경로를 모두 실행해야 합니다.
