...
라이트맵을 메인 텍스처와 같은 UV0으로 읽지 않는 이유는, 두 텍스처가 좌표를 사용하는 방식이 다르기 때문입니다. 메인 텍스처는 표면의 기본 무늬를 입히는 용도라서, 같은 좌표를 여러 표면에서 반복해서 읽어도 문제가 되지 않습니다. 만약 여러 표면이 같은 벽돌 무늬를 읽는다면, 각 표면에 같은 벽돌 패턴이 반복되어 보일 뿐입니다. 타일링이나 미러링을 UV0에서 자연스럽게 사용할 수 있는 것도 이 때문입니다.
