...
[물리 최적화 (1) - 물리 엔진의 실행 구조](/dev/unity/PhysicsOptimization-1/)에서는 PhysX의 물리 스텝과 Broadphase/Narrowphase, Rigidbody Sleep까지 물리 엔진의 비용 구조를 살펴보았습니다. 물리 비용은 한 가지 원인으로 결정되지 않습니다.

검사 대상 조합의 수, 시뮬레이션 빈도, 충돌 처리 방식에 따라 단계마다 별도의 비용이 더해집니다.
콜라이더가 늘어날수록 잠재적 조합의 수는 그 제곱에 가깝게 커집니다. Broadphase의 공간 분할 자료구조가 실제 비교 횟수를 좁혀주기는 하지만, 콜라이더 수가 많아질수록 자료구조를 갱신하고 관리하는 비용도 함께 무거워지게 됩니다.

이런 비용은 콜라이더 형상이나 Rigidbody Sleep을 다듬는 것만으로는 줄어들지 않습니다. 개별 오브젝트의 연산을 가볍게 만들 뿐, 검사할 조합의 수나 시뮬레이션 빈도는 그대로 두기 때문입니다. PhysX가 어떤 작업을, 언제, 어떻게 처리할지를 직접 조정해야 비용이 줄어듭니다.

이번 글에서는 검사 대상 조합을 걸러내는 Layer Collision Matrix, 시뮬레이션 시점을 정하는 Physics.Simulate 수동 제어, 그리고 차원에 맞는 엔진 선택을 다룹니다. 이어서 충돌과 메모리 처리를 가볍게 만드는 Trigger와 NonAlloc 패턴, Fixed Timestep과 정적 콜라이더 이동 같은 세부 설정까지 차례로 살펴봅니다.

---

## Layer Collision Matrix

실제 게임에서 모든 콜라이더가 서로 충돌해야 하는 것은 아닙니다. 같은 편 총알끼리는 서로 통과해야 하고, UI는 캐릭터의 움직임을 가로막지 않아야 합니다. Layer Collision Matrix에서 이런 충돌 검사를 비활성화하면, Broadphase 단계부터 검사 후보가 줄어듭니다.
...
**자동 시뮬레이션 (기본)** — `FixedUpdate` 타이밍마다 자동으로 물리 실행

**수동 시뮬레이션 (`SimulationMode.Script`)** — `Physics.Simulate()`를 호출한 시점에만 물리 실행

| 프레임 | 자동 시뮬레이션 | 수동 시뮬레이션 |
|---|---|---|
| 1 | 물리 ✓ ✓ | `Physics.Simulate(0.02f)` 호출 |
| 2 | 물리 ✓ | `Physics.Simulate(0.02f)` 호출 |
| 3 | 물리 ✓ ✓ ✓ | `Physics.Simulate(0.02f)` 호출 |
| **(메뉴 화면 진입)** | | |
| 4 | 물리 ✓ ← 필요 없는데 실행 | 호출 안 함 ← 물리 비용 0 |
| 5 | 물리 ✓ ← 필요 없는데 실행 | 호출 안 함 ← 물리 비용 0 |
...
<svg viewBox="0 0 580 290" xmlns="http://www.w3.org/2000/svg" style="max-width: 580px; width: 100%;">
  <text x="290" y="20" text-anchor="middle" font-family="sans-serif" font-size="13" font-weight="bold" fill="currentColor">Collider vs Trigger</text>

  <text x="145" y="50" text-anchor="middle" font-family="sans-serif" font-size="12" font-weight="bold" fill="currentColor">일반 Collider (Is Trigger = false)</text>
  ...
  <text x="147" y="138" text-anchor="middle" font-family="sans-serif" font-size="11" fill="currentColor">충돌 시 밀려남</text>
  <text x="147" y="165" text-anchor="middle" font-family="sans-serif" font-size="11" fill="currentColor">접촉점 계산 → 물체 분리 → 반발/마찰 적용</text>
  ...
  <text x="435" y="138" text-anchor="middle" font-family="sans-serif" font-size="11" fill="currentColor">겹쳐도 통과</text>
  <text x="435" y="165" text-anchor="middle" font-family="sans-serif" font-size="11" fill="currentColor">겹침 감지만 수행, 물리적 반응 없음</text>
...
<svg viewBox="0 0 620 230" xmlns="http://www.w3.org/2000/svg" style="max-width: 620px; width: 100%;">
  ...
  <text x="20" y="58" font-family="sans-serif" font-size="12" font-weight="bold" fill="currentColor">일반 Collider</text>
  ...
  <g font-family="sans-serif" font-size="10" fill="currentColor">
    ...
    <rect x="210" y="72" width="80" height="30" rx="4" fill="currentColor" fill-opacity="0.06" stroke="currentColor" stroke-width="1.2"/>
    <text x="250" y="91" text-anchor="middle">접촉점 계산</text>

    <rect x="305" y="72" width="80" height="30" rx="4" fill="currentColor" fill-opacity="0.06" stroke="currentColor" stroke-width="1.2"/>
    <text x="345" y="91" text-anchor="middle">충돌 해소</text>

    <rect x="400" y="72" width="90" height="30" rx="4" fill="currentColor" fill-opacity="0.06" stroke="currentColor" stroke-width="1.2"/>
    <text x="445" y="91" text-anchor="middle">위치/속도 갱신</text>

    <rect x="505" y="72" width="100" height="30" rx="4" fill="currentColor" fill-opacity="0.12" stroke="currentColor" stroke-width="1.5"/>
    <text x="555" y="91" text-anchor="middle">OnCollisionEnter</text>
  </g>
  ...
  <g stroke="currentColor" stroke-width="1.2" fill="currentColor">
    ...
    <line x1="195" y1="87" x2="208" y2="87"/>
    <polygon points="211,87 205,84 205,90"/>
    <line x1="290" y1="87" x2="303" y2="87"/>
    <polygon points="306,87 300,84 300,90"/>
    <line x1="385" y1="87" x2="398" y2="87"/>
    <polygon points="401,87 395,84 395,90"/>
    <line x1="490" y1="87" x2="503" y2="87"/>
    <polygon points="506,87 500,84 500,90"/>
  </g>

  <text x="20" y="148" font-family="sans-serif" font-size="12" font-weight="bold" fill="currentColor">Trigger</text>

  <g font-family="sans-serif" font-size="10" fill="currentColor">
    <rect x="20" y="162" width="80" height="30" rx="4" fill="currentColor" fill-opacity="0.06" stroke="currentColor" stroke-width="1.2"/>
    <text x="60" y="181" text-anchor="middle">Broadphase</text>

    <rect x="115" y="162" width="80" height="30" rx="4" fill="currentColor" fill-opacity="0.06" stroke="currentColor" stroke-width="1.2"/>
    <text x="155" y="181" text-anchor="middle">Narrowphase</text>

    <rect x="210" y="162" width="80" height="30" rx="4" fill="currentColor" fill-opacity="0.06" stroke="currentColor" stroke-width="1.2"/>
    <text x="250" y="181" text-anchor="middle">겹침 확인</text>

    <rect x="305" y="162" width="185" height="30" rx="4" fill="none" stroke="currentColor" stroke-width="1.2" stroke-dasharray="4,3" stroke-opacity="0.55"/>
    <text x="397" y="181" text-anchor="middle" fill-opacity="0.65">접촉점 계산 · 충돌 해소 · 위치/속도 갱신 생략</text>

    <rect x="505" y="162" width="100" height="30" rx="4" fill="currentColor" fill-opacity="0.12" stroke="currentColor" stroke-width="1.5"/>
    <text x="555" y="181" text-anchor="middle">OnTriggerEnter</text>
  </g>

  <g stroke="currentColor" stroke-width="1.2" fill="currentColor">
    <line x1="100" y1="177" x2="113" y2="177"/>
    <polygon points="116,177 110,174 110,180"/>
    <line x1="195" y1="177" x2="208" y2="177"/>
    <polygon points="211,177 205,174 205,180"/>
    <line x1="290" y1="177" x2="303" y2="177"/>
    <polygon points="306,177 300,174 300,180"/>
    <line x1="490" y1="177" x2="503" y2="177" stroke-dasharray="3,2"/>
    <polygon points="506,177 500,174 500,180"/>
  </g>
</svg>
</div>
...
| 사례 | 상황 | 물리 반응을 피해야 하는 이유 | 해결 |
|---|---|---|---|
| 아이템 획득 | 플레이어가 아이템 위를 지나가면 획득 | 아이템이 플레이어를 밀어내면 안 됨 | Trigger로 겹침 감지 후 아이템 제거 |
| 영역 진입 감지 | 특정 구역에 플레이어가 들어오면 이벤트 발생 | 보이지 않는 벽이 플레이어를 막으면 안 됨 | Trigger 콜라이더로 영역 설정 |
| 공격 범위 판정 | 검의 휘두름 범위에 적이 있는지 확인 | 검이 적을 물리적으로 밀어내는 것은 부자연스러움 | Trigger로 범위 겹침 확인 후 데미지 적용 |
| AI 시야 탐지 | 적 AI의 시야 범위에 플레이어가 들어왔는지 확인 | 물리적 반응 불필요 | 큰 Sphere Trigger로 탐지 범위 설정 |
...
| Fixed Timestep | 프레임당 횟수 |
|---|---|
| 0.02초 | 약 1.67회 (대부분 1~2회) |
| 0.04초 | 약 0.83회 (대부분 0~1회) |
...
→ 정적 콜라이더는 이동하지 않는다고 최적화되어 있으므로, 이 재구성 비용은 동적 콜라이더의 AABB 갱신보다 훨씬 높음
...
| # | 전략 | 효과 | 구현 |
|---|---|---|---|
| 1 | Layer Collision Matrix 설정 | 불필요한 충돌 쌍 제거 | 설정만 변경, 코드 수정 불필요 (프로젝트 초기에 레이어 구조와 함께 설정하면 이상적) |
| 2 | 정적 콜라이더에 Rigidbody(Kinematic) 부착 | 정적 콜라이더 이동으로 인한 재구성 비용 제거 | 컴포넌트 추가만 |
| 3 | Trigger 활용 | 물리적 반응이 불필요한 곳에서 충돌 해소 생략 | `Is Trigger` 체크만 변경 |

**높은 효과, 중간 구현 비용**

| # | 전략 | 효과 | 구현 |
|---|---|---|---|
| 4 | NonAlloc 물리 쿼리 | 매 프레임 힙 할당 제거 | 코드 수정 필요, 패턴은 단순 |
| 5 | 2D 게임에 2D 물리 사용 | z축 연산 제거 | 프로젝트 초기에 결정하면 비용 없음 |
| 6 | Fixed Timestep 조정 | 물리 스텝 빈도 감소 | 값 변경만 하면 되지만 정밀도 영향 확인 필요 |

**중간 효과, 높은 구현 비용**

| # | 전략 | 효과 | 구현 |
|---|---|---|---|
| 7 | `Physics.Simulate` 수동 제어 | 불필요한 시점에 물리 완전 중단 | 타이밍 관리 코드 필요 |
| 8 | Solver Iteration 개별 조정 | 오브젝트별 정밀도·비용 최적화 | 각 오브젝트의 요구사항 분석 필요 |
