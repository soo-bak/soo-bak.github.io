...
따라서 Fixed Timestep은 물리를 얼마나 드문 간격으로 계산해도 되는지를 정하는 값에 가깝습니다. 물리 퍼즐, 차량, 액션 게임처럼 충돌 반응이 게임플레이에 직접 영향을 주는 경우에는 작은 변화도 체감될 수 있습니다. 물리 상호작용이 단순한 게임이라면, Profiler로 비용 변화를 확인하고 플레이 테스트에서 충돌 반응이 허용 가능한지 검증한 뒤 값을 늘리는 방식을 검토할 수 있습니다.
