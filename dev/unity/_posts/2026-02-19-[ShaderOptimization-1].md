...
세 사이클 중 가장 큰 값을 만든 작업이 곧 그 셰이더의 병목이므로, 위 표에서 해당 병목 유형의 최적화 방향을 선택해 적용하면 가장 효과적으로 성능을 개선할 수 있습니다.
...
`pow()`, `sin()`, `cos()` 같은 함수는 단순한 덧셈이나 곱셈보다 처리 비용이 큽니다. 덧셈과 곱셈은 GPU의 기본 산술 명령으로 처리되지만, 삼각 함수나 거듭제곱은 보통 여러 명령으로 근사하거나 특수 함수 유닛을 사용하기 때문입니다.
이런 연산이 프래그먼트 셰이더에서 넓은 화면 영역에 반복되면, 셰이더 비용이 ALU 연산 쪽으로 빠르게 커질 수 있습니다.

입력 범위가 제한되어 있고 결과가 연속적으로 변하는 함수라면 **LUT(Look-Up Table)** 를 사용할 수 있습니다. LUT는 함수 결과를 미리 계산해 텍스처에 저장해 두고, 런타임에는 수학 함수를 다시 계산하는 대신 텍스처에서 값을 읽는 방식입니다.
...
// LUT 조회
half shininess01 = saturate(_Shininess / _MaxShininess);
half specular = SAMPLE_TEXTURE2D(
    _SpecLUT,
    sampler_SpecLUT,
    half2(saturate(NdotH), shininess01)
).r;
```

다만 LUT는 수학 연산을 텍스처 조회로 바꾸는 방식이므로 항상 더 빠르지는 않습니다. LUT용 텍스처를 따로 보관해야 하고, 런타임에는 함수 계산 대신 텍스처 샘플링이 추가됩니다.
그래서 LUT는 고비용 함수가 넓은 화면 영역에서 반복될 때 의미가 있습니다. 이 경우에도 실제로는 직접 계산과 텍스처 조회 중 어느 쪽이 더 저렴한지 프로파일링으로 확인하는 편이 안전합니다.

### 텍스처 수와 채널 패킹

PBR(Physically Based Rendering) 머티리얼은 표면의 색, 법선, 금속성, 거칠기, 차폐 정도를 각각 다른 텍스처로 표현하는 경우가 많습니다. 이 값들을 각각 별도 텍스처로 두면, 프래그먼트 셰이더는 한 픽셀을 계산할 때 여러 번 텍스처 샘플링을 수행해야 합니다.
샘플링 횟수가 늘어나면 텍스처 메모리 접근이 많아지고, 그만큼 메모리 대역폭 부담도 커집니다.

이 중 일부 값은 조건이 맞으면 하나의 텍스처에 묶을 수 있습니다. Metallic, Roughness, Occlusion은 각각 픽셀마다 숫자 하나로 표현되는 값입니다. Albedo 텍스처는 R, G, B 채널이 함께 하나의 색을 표현하지만, 이런 스칼라 데이터는 채널 하나만으로 의미가 완성됩니다. 그래서 하나의 RGBA 텍스처에서 서로 다른 채널에 나누어 담을 수 있습니다.
예를 들어 R 채널에는 Metallic, G 채널에는 Roughness, B 채널에는 Occlusion을 저장하면 한 번의 텍스처 샘플링으로 세 값을 함께 가져올 수 있습니다.
이렇게 여러 단일 채널 데이터를 하나의 텍스처에 묶는 방식을 **채널 패킹(Channel Packing)**이라고 합니다.
...
```hlsl
half4 packed = SAMPLE_TEXTURE2D(_PackedTex, sampler_PackedTex, uv);
half metallic  = packed.r;
half roughness = packed.g;
half occlusion = packed.b;
half detailMask = packed.a;
```

<br>

어떤 값을 어느 채널에 넣을지는 프로젝트의 머티리얼 규칙이나 렌더 파이프라인의 텍스처 규칙에 맞춰 정합니다. 예를 들어 특정 파이프라인이 Metallic을 R 채널, Smoothness를 A 채널에서 읽도록 정해 두었다면 셰이더와 텍스처 제작 규칙도 그에 맞춰야 합니다.

채널 패킹의 장점은 한 번의 텍스처 샘플링으로 여러 값을 함께 읽는 데 있습니다. 한 번 샘플링하면 RGBA 채널은 모두 같은 UV 위치에서 읽히므로, 묶인 값들이 같은 UV와 같은 해상도로 사용되어도 문제가 없어야 합니다. 또한 같은 텍스처 안의 채널들은 필터링, 밉맵, 압축 형식, sRGB 설정을 함께 사용합니다.

예를 들어 Occlusion은 낮은 해상도로도 충분하지만 Roughness는 더 높은 해상도가 필요할 수 있습니다. 이 둘을 하나로 묶으면 Roughness에 맞춰 전체 텍스처 해상도를 높여야 하므로, Occlusion 채널까지 불필요하게 큰 텍스처에 실리게 됩니다. 반대로 낮은 해상도에 맞추면 Roughness 품질이 부족해질 수 있습니다.

UV 사용 방식이 다른 경우도 주의해야 합니다. 텍스처 샘플링 호출 하나에는 UV 좌표 하나만 전달할 수 있고, 그 결과로 반환되는 R, G, B, A 채널은 모두 그 UV 위치에서 읽힌 값입니다. 예를 들어 Metallic과 Roughness는 기본 UV로 읽고, Detail Mask는 더 촘촘하게 반복되는 tiling UV로 읽어야 할 수 있습니다. 이때 Detail Mask만 다른 UV로 가져오려면 같은 패킹 텍스처를 tiling UV로 한 번 더 샘플링해야 합니다.
이렇게 되면 채널 패킹으로 줄이려던 샘플링 이점이 줄어듭니다. UV 사용 방식이 다른 값은 별도 텍스처로 두는 편이 더 단순할 수 있습니다.

패킹 텍스처는 보통 색상이 아니라 데이터로 취급해야 합니다. Metallic, Roughness, Occlusion 값은 화면에 보이는 색이 아니라 셰이더 계산에 들어가는 수치이므로, sRGB 보정이 적용되면 값이 의도와 다르게 변할 수 있습니다. 이런 텍스처는 Linear 데이터로 읽히도록 설정해야 합니다.

> sRGB와 Linear 텍스처의 차이는 [색과 빛 (2) - 색 표현과 색공간](/dev/unity/ColorAndLight-2/)에서, Unity의 텍스처 임포트 설정은 [Unity 에셋 시스템 (1) - Asset Import Pipeline](/dev/unity/UnityAsset-1/)에서 자세히 다룹니다.

압축 형식도 함께 확인해야 합니다. 채널 패킹을 하면 R, G, B, A 채널이 하나의 텍스처로 저장되므로, 모든 채널이 같은 압축 포맷과 압축 품질을 공유합니다. 따라서 각 채널의 데이터가 압축 오차를 얼마나 견딜 수 있는지 함께 봐야 합니다.

예를 들어 Roughness는 표면이 얼마나 매끄럽거나 거친지를 나타내며, 반사 하이라이트의 폭과 선명도에 직접 영향을 줍니다. 압축으로 Roughness 값에 블록 노이즈나 계단 현상이 생기면 하이라이트가 얼룩지거나 표면 질감이 의도와 다르게 보일 수 있습니다. 반면 Metallic은 많은 머티리얼에서 금속/비금속을 나누는 값처럼 0 또는 1에 가깝게 쓰이므로, 같은 정도의 압축 오차가 상대적으로 덜 눈에 띌 수 있습니다.

따라서 채널 패킹은 샘플링 횟수를 줄이는 대신, 같은 해상도와 압축 설정을 공유해도 되는 값끼리 묶는 방식으로 적용하는 편이 안전합니다.

<br>

### Warp/Wavefront 내 분기 발산

GPU는 프래그먼트 셰이더를 프래그먼트 하나씩 완전히 따로 실행하지 않습니다. 여러 프래그먼트의 실행을 하나의 그룹으로 묶고, 그 그룹 안에서 같은 명령을 함께 처리합니다. NVIDIA에서는 이 그룹을 Warp, AMD에서는 Wavefront라고 부릅니다.

`if` 조건이 그룹 전체에서 같은 결과를 내면 처리할 블록은 하나입니다. 모두 `true`라면 `true` 블록만 처리하고, 모두 `false`라면 `false` 블록만 처리하면 됩니다.

문제는 같은 그룹 안에서 조건 결과가 섞일 때입니다. 예를 들어 어떤 프래그먼트는 `brightness > 0.5`가 참이고, 바로 옆 프래그먼트는 거짓일 수 있습니다. 이때 GPU는 그룹을 즉시 둘로 나누어 각각 완전히 독립적으로 실행하지 않습니다.

보통은 `true` 블록을 처리할 때 조건이 참인 프래그먼트만 활성화하고, `false` 블록을 처리할 때 조건이 거짓인 프래그먼트만 활성화합니다. 이렇게 같은 Warp/Wavefront 안에서 조건 결과가 달라져 두 블록을 모두 처리해야 하는 상황을 **분기 발산(branch divergence)**이라고 합니다.

분기 발산이 생기면 `if`문이 있어도 그룹 전체 기준으로는 두 블록의 명령을 모두 거치게 됩니다. 그래서 프래그먼트마다 결과가 자주 달라지는 조건문은 기대한 만큼 비용을 줄이지 못할 수 있습니다.

반대로 조건이 머티리얼 값, 오브젝트 플래그, 품질 설정처럼 한 드로우 안에서 동일하게 평가되면 그룹 안에서 결과가 거의 달라지지 않습니다. 이런 조건은 한쪽 블록만 처리하면 되므로 발산 비용이 작습니다.
...
```hlsl
// 조건 결과가 프래그먼트마다 달라지면 분기 발산 가능
if (brightness > 0.5)
{
    color *= 2.0;
}

// 단순한 값 선택은 분기 없이 표현 가능
half factor = lerp(1.0, 2.0, step(0.5, brightness));
color *= factor;
```

위 예시에서 `if`문이 하는 일은 조건에 따라 1.0 또는 2.0을 곱하는 것입니다. 이처럼 단순히 값 하나를 정하는 경우라면, `step()`으로 0 또는 1을 만들고 `lerp()`로 곱할 값을 선택할 수 있습니다.

다만 `lerp()`는 코드 블록을 선택해서 실행하는 문법이 아니라, 두 값 사이의 결과를 계산하는 연산입니다. 선택해야 하는 값 자체를 만들기 위해 텍스처 샘플링이나 복잡한 계산이 필요하다면, `if`문을 없앴다고 해서 그 비용까지 사라지는 것은 아닙니다.

따라서 분기 최적화는 `if`문을 무조건 `lerp()`로 바꾸는 작업이 아닙니다. 분기 안에서 다루는 것이 가벼운 값인지, 아니면 실제로 비용이 큰 계산인지에 따라 선택해야 합니다.

---

## 마무리

- 셰이더 비용은 ALU 연산, 텍스처 샘플링, 메모리 대역폭이 함께 결정합니다. 어떤 쪽이 병목인지에 따라 최적화 방향도 달라집니다.
- HLSL 코드는 플랫폼별 컴파일러와 GPU 드라이버를 거쳐 실제 GPU 명령으로 바뀝니다. 따라서 최종 성능은 대상 기기에서 확인해야 합니다.
- 프래그먼트 셰이더는 프래그먼트 수와 오버드로우에 비례해 반복 실행됩니다. 작은 연산 차이도 넓은 화면 영역에서는 큰 비용으로 누적될 수 있습니다.
- `half`는 값의 범위와 정밀도가 충분할 때만 사용해야 합니다. 실제 이점은 대상 GPU가 16비트 연산을 효율적으로 처리할 때 나타납니다.
- LUT, 채널 패킹, 분기 대체, 버텍스 셰이더로의 연산 이동은 비용을 줄일 수 있지만, 각각 텍스처 비용, 데이터 조건, 분기 발산, 보간 오차를 함께 확인해야 합니다.

셰이더 최적화는 화면 품질을 단순히 낮추는 작업이 아닙니다. 같은 결과를 더 적은 연산, 더 적은 샘플링, 더 적절한 데이터 표현으로 처리하도록 조정하는 과정입니다. 명령어 수, 정밀도, 텍스처 수, 계산 위치를 바꿀 때는 성능과 화면 품질을 함께 확인해야 합니다.

다만 셰이더 코드 자체를 가볍게 만드는 것만으로 전체 렌더링 비용이 끝나는 것은 아닙니다. Unity에서는 키워드 조합에 따라 하나의 셰이더 소스에서 여러 **셰이더 배리언트(variant)**가 만들어질 수 있고, 배리언트 수가 많아지면 빌드 시간, 메모리, 로딩 시간에도 영향을 줍니다.

[Part 2](/dev/unity/ShaderOptimization-2/)에서는 셰이더 배리언트가 늘어나는 구조와 키워드 관리, 스트리핑 전략을 다룹니다.
