...
보통은 `true` 블록을 처리할 때 조건이 참인 프래그먼트만 활성화하고, `false` 블록을 처리할 때 조건이 거짓인 프래그먼트만 활성화합니다. 이렇게 같은 Warp/Wavefront 안에서 조건 결과가 갈려 두 블록을 모두 처리해야 하는 상황을 **분기 발산(branch divergence)**이라고 합니다.

분기 발산이 생기면 `if`문이 있어도 그룹 전체 기준으로는 두 블록의 명령을 모두 거치게 됩니다. 그래서 프래그먼트마다 결과가 자주 갈리는 조건문은 기대한 만큼 비용을 줄이지 못할 수 있습니다.

반대로 조건이 머티리얼 값, 오브젝트 플래그, 품질 설정처럼 한 드로우 안에서 동일하게 평가되면 그룹 안에서 결과가 잘 갈리지 않습니다. 이런 조건은 한쪽 블록만 처리하면 되므로 발산 비용이 작습니다.
...
위 예시에서 `if`문이 하는 일은 조건에 따라 1.0 또는 2.0을 곱하는 것입니다. 이처럼 단순히 값 하나를 고르는 경우라면, `step()`으로 0 또는 1을 만들고 `lerp()`로 곱할 값을 선택할 수 있습니다.
...
따라서 분기 최적화는 `if`문을 무조건 `lerp()`로 바꾸는 작업이 아닙니다. 분기 안에서 고르는 것이 가벼운 값인지, 아니면 실제로 비용이 큰 계산인지에 따라 선택해야 합니다.
