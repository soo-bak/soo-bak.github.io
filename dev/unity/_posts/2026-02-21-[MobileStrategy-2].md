...
레벨을 선택할 때는 빌드 크기만 보지 않는 것이 좋습니다. Minimal은 가장 안전하지만 크기 감소 효과가 제한적이고, High는 크기를 더 줄일 수 있지만 런타임에서만 접근하는 코드가 제거될 가능성이 커집니다. 일반적인 모바일 프로젝트라면 Minimal이나 Medium에서 시작하고, 빌드 크기와 기능 테스트 결과를 보며 조정하는 편이 적절합니다.
