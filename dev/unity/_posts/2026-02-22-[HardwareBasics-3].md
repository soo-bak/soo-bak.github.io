...
초기 3D 가속기는 CPU가 하던 그래픽 작업을 한 번에 모두 가져간 장치가 아니었습니다. 게임 로직과 정점 변환은 여전히 CPU가 처리했고, 3D 가속기는 래스터화, 텍스처 매핑, 깊이 테스트, 프레임버퍼 기록처럼 픽셀마다 반복되는 작업을 CPU로부터 넘겨받는 역할에 가까웠습니다.
