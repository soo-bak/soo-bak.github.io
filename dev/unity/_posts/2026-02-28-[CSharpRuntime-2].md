...
[C# 런타임 기초 (1) - 값 타입과 참조 타입](/dev/unity/CSharpRuntime-1/)에서는 값 타입이 데이터를 직접 담아 스택에 놓이고, 참조 타입은 힙에 객체가 만들어진 뒤 변수에는 주소만 담긴다는 점을 다루었습니다. 이 타입 구분이 메모리 배치와 GC 비용을 가릅니다.

앞 글이 데이터가 메모리 어디에 놓이는가를 다뤘다면, 이 글은 한 걸음 더 나아가 작성한 코드가 어떻게 CPU에서 실행되는가를 따라갑니다. 이 과정을 맡는 것이 **런타임 시스템(Runtime System)**입니다. "런타임"이라는 말 자체는 "실행 시점"을 뜻하지만, 여기서는 코드 로딩과 기계어 변환, 메모리 관리, 예외 처리까지 떠맡는 소프트웨어 계층을 가리킵니다. 한 단어가 두 뜻으로 쓰이는 까닭은, 이 계층이 곧 실행 시점에 동작하는 시스템이기 때문입니다.

CPU가 게임 로직을 실행하려면 C# 소스 코드가 기계어로 바뀌어야 하는데, C#은 C++처럼 소스가 곧장 기계어가 되는 구조가 아닙니다. C# 컴파일 결과물은 **IL(Intermediate Language)**이라는 중간 표현이고, 이 IL을 언제 어떤 방식으로 기계어로 바꾸느냐에 따라 런타임의 성격이 갈립니다.

이 변환 방식의 차이는 게임 성능에 곧장 영향을 줍니다. 같은 C# 코드라도 런타임이 적용하는 최적화 수준(인라이닝, 루프 벡터화 등)에 따라 생성되는 기계어 품질이 달라져 CPU 연산 속도가 바뀌고, iOS처럼 실행 시점에 새 기계어를 만드는 것 자체를 막는 플랫폼에서는 특정 방식을 아예 쓸 수 없습니다.

Unity는 이 변환을 두 가지 런타임 방식 중 하나로 처리합니다. 실행 시점에 IL을 기계어로 바꾸는 **Mono(JIT)**와, 빌드 시점에 IL을 C++로 옮긴 뒤 네이티브 컴파일하는 **IL2CPP(AOT)**입니다. 두 방식은 빌드 시간과 실행 성능, 플랫폼 호환성에서 서로 다른 특성을 가지며, 대상 플랫폼에 따라 선택이 묶이기도 합니다.
...
C#은 **컴파일 언어**라서, 코드를 실행하기 전에 컴파일러가 먼저 변환을 한 번 거칩니다. 다만 그 결과물이 곧장 기계어가 되지는 않고 중간 단계를 하나 지납니다.

C# 컴파일러(Roslyn)는 소스 코드를 **IL(Intermediate Language, 중간 언어)**이라는 바이트코드로 바꾸는데, IL은 CIL(Common Intermediate Language)이나 MSIL(Microsoft Intermediate Language)로도 불립니다. IL은 기계어가 아니라 CPU가 곧바로 실행하지 못하는, 플랫폼에 독립적인 중간 표현입니다.
...
IL을 중간에 끼워 두는 이유는 .NET의 설계 원칙인 **플랫폼 독립성**에 있습니다. C#으로 짠 코드를 Windows, Linux, macOS, iOS, Android에서 모두 돌리려면 플랫폼마다 기계어가 달라 따로 컴파일해야 하는데, IL이라는 중간 단계를 두면 C# 컴파일러는 플랫폼과 무관한 IL만 만들고 기계어 변환은 각 플랫폼의 런타임이 떠맡습니다. 컴파일러와 런타임의 역할이 이렇게 나뉩니다.
...
IL은 스택을 기반으로 한 명령어 집합입니다. 간단한 C# 코드를 IL로 바꿔 보면 그 성격이 한눈에 드러납니다.
...
IL은 바이트코드라서 각 명령어(opcode)는 1~2바이트로 인코딩되고, 명령어에 따라 피연산자(operand)가 뒤에 붙습니다. 위 예시의 `ldarg.0`은 첫 번째 인자를 **평가 스택(Evaluation Stack)**에 올리는 명령이고, `add`는 스택 맨 위의 두 값을 꺼내 더한 결과를 다시 스택에 올리는 명령입니다.

평가 스택은 IL 명령어가 연산에 쓸 값을 잠깐 쌓아 두는 논리적 자료구조입니다. CPU에서 레지스터가 피연산자를 담는 역할과 비슷하지만, 레지스터는 이름으로 직접 집어 쓰는 반면 평가 스택은 LIFO(후입선출)로만 드나듭니다.

IL은 그 자체로는 CPU가 실행하지 못하므로, 실행 전에 반드시 해당 플랫폼의 기계어로 바뀌어야 합니다. 이 변환을 맡는 것이 **런타임**이고, 변환 방식에 따라 **JIT(Just-In-Time)**와 **AOT(Ahead-Of-Time)**로 갈립니다.

---

## JIT 컴파일 — Mono 런타임

**JIT(Just-In-Time) 컴파일**은 프로그램을 실행하는 도중에 IL을 기계어로 바꾸는 방식이고, Unity에서 이 변환을 맡는 런타임이 **Mono**입니다. 게임이 돌다가 어떤 메서드가 처음 호출되면 Mono가 그 메서드의 IL을 읽어 기계어로 바꾸고, 변환된 기계어를 메모리에 캐시해 둡니다. 그래서 같은 메서드가 두 번째로 불릴 때는 변환 없이 캐시된 기계어를 곧장 실행합니다.
...
**빌드 시간이 짧습니다.** C# 컴파일러가 IL을 만들면 빌드는 거기서 끝납니다. 기계어 변환은 실행 시점에 일어나 빌드에 끼지 않으므로, Unity 에디터에서 Play를 누르면 IL→C++→기계어로 이어지는 AOT 변환 없이 IL을 그대로 올려 즉시 실행됩니다. 덕분에 코드를 고치고 곧장 돌려 보는 빠른 이터레이션이 가능합니다.

**첫 호출 때 변환 비용이 듭니다.** 메서드가 처음 불릴 때 JIT 컴파일러가 IL을 읽어 기계어를 만드는 시간이 듭니다. 게임 시작 직후나 새 씬을 막 불러온 직후에 처음 호출되는 메서드들이 한꺼번에 JIT 컴파일되면 그 순간 프레임 시간이 늘 수 있습니다. 한 번 컴파일된 메서드는 캐시되므로 그다음부터는 이 비용이 들지 않습니다.
...
**최적화 수준에 한계가 있습니다.** JIT 컴파일러는 실행 도중에 기계어를 만들어야 해서 변환에 쓸 시간이 빠듯합니다. 변환이 길어지면 그만큼 게임 실행이 밀리기 때문입니다.

컴파일러가 기계어를 뽑을 때 쓰는 대표적인 최적화로는, 짧은 메서드 본문을 호출 지점에 직접 끼워 넣어 호출 비용을 없애는 **인라이닝**, 반복문 연산을 CPU의 SIMD 명령으로 묶어 한 번에 여러 데이터를 처리하는 **루프 벡터화**, 닿을 수 없는 코드를 지워 바이너리 크기와 분기 비용을 줄이는 **데드 코드 제거**가 있습니다. Mono JIT는 인라이닝과 데드 코드 제거를 기초 수준에서만 하고, 루프 벡터화 같은 고비용 최적화는 거의 못 합니다. AOT 컴파일러가 빌드 시간의 여유로 이런 최적화를 폭넓게 적용하는 반면, JIT 컴파일러는 빠르게 "쓸 만한" 기계어를 내놓는 데 초점을 둡니다.

**런타임에 새 코드를 만들어 냅니다.** JIT 컴파일은 실행 도중 메모리에 실행 가능한 기계어를 새로 얹는 과정입니다. 운영체제는 메모리 영역마다 읽기·쓰기·실행 권한을 따로 관리하는데, JIT 컴파일러가 메모리에 기계어를 쓴 뒤 그 영역에 실행 권한을 줘야 CPU가 그 코드를 돌릴 수 있습니다. iOS처럼 서드파티 앱이 메모리에 실행 권한을 동적으로 주는 것을 막는 플랫폼에서는 이 단계가 통째로 막히므로 JIT 방식을 쓸 수 없습니다.

---

## AOT 컴파일 — IL2CPP

**AOT(Ahead-Of-Time) 컴파일**은 빌드 시점에 IL을 기계어로 미리 바꿔 두는 방식입니다. 실행할 때는 기계어가 이미 다 만들어져 있어 JIT 같은 변환 단계가 없습니다. Unity에서 이 AOT를 맡는 파이프라인이 **IL2CPP**입니다.

IL2CPP는 "IL to C++"의 약자로 Unity가 직접 만든 변환 도구입니다. IL을 곧바로 기계어로 바꾸는 대신, 먼저 **C++ 소스 코드**로 옮기고 그 C++를 플랫폼의 네이티브 컴파일러(Clang, GCC, MSVC 등)로 컴파일해 최종 기계어를 뽑습니다.
...
IL2CPP가 굳이 IL을 C++로 한 번 거치는 이유는 실용적인 비용 절감에 있습니다. iOS·Android·Windows·WebGL 같은 플랫폼마다 기계어 코드 생성기(code generator)를 직접 만드는 대신, 이미 각 플랫폼에 잘 갖춰진 C++ 컴파일러(Clang, GCC, MSVC 등)를 빌려 쓰는 것입니다. 수십 년 동안 다듬어진 C++ 컴파일러의 코드 생성·최적화 역량을 그대로 활용하는 구조입니다.

---

## Mono vs IL2CPP 비교

앞에서 Mono(JIT)와 IL2CPP(AOT)가 각각 어떻게 동작하는지 다뤘습니다. 두 런타임의 주요 특성을 한 표로 모으면 다음과 같습니다.
...
**빌드 시간.** Mono 빌드는 C# 컴파일러가 IL을 만들면 끝나지만, IL2CPP 빌드는 여기에 IL→C++ 변환과 C++ 컴파일이 더 붙어 대규모 프로젝트에서는 수십 분까지 늘기도 합니다.

이 시간을 줄이려면 이전과 같은 디렉터리에 빌드해 C++ 컴파일러가 바뀐 파일만 다시 컴파일하는 **증분 빌드(Incremental Build)**를 쓰면 됩니다. IL2CPP Code Generation 옵션을 Faster (Smaller) Builds로 두면 생성되는 C++ 코드량이 줄어 빌드 시간이 더 짧아집니다.

**실행 성능.** Mono의 JIT는 실행 도중 빠르게 기계어를 뽑아야 해서 복잡한 최적화에 시간을 들이지 못합니다. 반면 IL2CPP는 빌드 시점에 C++ 컴파일러가 충분히 시간을 들여 최적화하므로, 만들어지는 기계어 품질이 더 높습니다.

그래서 수학 연산이 많은 게임 로직이나 물리 시뮬레이션, AI 경로 탐색처럼 연산·루프가 몰린 코드에서는 IL2CPP가 Mono보다 빠르고, 1.5~2배의 속도 향상이 보고되기도 합니다. 다만 이 수치는 코드 패턴에 따라 편차가 크고, I/O 중심 코드에서는 차이가 거의 없을 수 있습니다.

**코드 보호.** Mono 빌드에는 IL이 그대로 들어갑니다. IL은 클래스명·메서드명·변수명처럼 고수준 언어에 가까운 정보를 그대로 품고 있어, ILSpy나 dnSpy 같은 도구로 쉽게 역컴파일됩니다.

IL2CPP 빌드에서는 C# 코드가 C++를 거쳐 기계어로 바뀌면서 원본 구조가 상당히 흩어집니다. 완전한 보호는 아니어도 역공학 난이도가 한층 높아집니다.
...
함수를 호출하면 매개변수 전달과 스택 프레임 생성, 반환 같은 오버헤드가 따르는데, 함수 본문이 짧을수록 이 오버헤드가 실제 연산 비용에 비해 커집니다. 인라인화는 함수를 호출하는 대신 호출 지점에 함수 본문을 그대로 끼워 넣어 이 오버헤드를 없앱니다.
...
위 예시에서 `multiplier`가 늘 `1.0f`라면 컴파일러는 `baseSpeed * 1.0f`를 `baseSpeed`로 단순화할 수 있습니다. `GetSpeed()`가 별개 함수로 남아 있을 때는 이런 단순화가 불가능하지만, 인라인화로 호출자의 값이 드러나면 가능해집니다.
...
단순한 `if (false)`만이 아니라, 컴파일러는 상수 전파로 값이 미리 확정되는 조건문까지 분석해 닿을 수 없는 분기를 걷어냅니다. IL2CPP 빌드에서는 이 분석을 C++ 컴파일러의 최적화 패스가 맡으므로, JIT보다 더 넓은 범위의 데드 코드를 찾아 제거합니다.
...
IL2CPP의 이점은 실행 성능에 그치지 않습니다. Unity는 IL2CPP 빌드 과정에서 **코드 스트리핑(Code Stripping)**도 함께 수행하는데, 게임이 실제로 쓰지 않는 코드를 골라 최종 빌드에서 덜어내는 작업입니다.

.NET 라이브러리에는 수많은 클래스와 메서드가 들어 있지만 대부분의 게임은 그중 일부만 씁니다. 코드 스트리핑은 진입점(entry point)에서 출발해 참조를 따라가며, 실제로 닿는 코드만 남기고 나머지를 들어냅니다.
...
코드 스트리핑으로 줄어드는 크기는 수 MB에서 수십 MB에 이릅니다. 모바일에서는 이 차이가 특히 큰데, Google Play 기준으로 앱 크기가 200 MB를 넘으면 다운로드 단계에서 경고가 떠 이탈률이 올라가기 때문입니다.

다만 코드 스트리핑에는 함정이 하나 있습니다. 리플렉션(reflection)으로 동적으로 접근하는 코드는 정적 분석에 참조가 잡히지 않아 통째로 제거될 수 있습니다. 예를 들어 `Type.GetType("MyClass")`처럼 문자열로 타입을 찾으면 스트리핑 도구는 `MyClass`가 쓰인다는 사실을 알아채지 못하고, 그렇게 제거된 코드를 런타임에 부르려 하면 `MissingMethodException`이나 `TypeLoadException`이 납니다.

JSON 직렬화 라이브러리(예: `JsonUtility`, Newtonsoft.Json)도 리플렉션으로 프로퍼티에 접근하므로 같은 문제를 겪을 수 있어, 직렬화 대상 타입은 더욱 조심해야 합니다.

이런 제거를 막으려면 Unity의 `link.xml`에 보존할 타입이나 어셈블리를 적어 주면 됩니다. `link.xml`은 프로젝트 Assets 폴더에 두는 XML 파일로 스트리핑에서 제외할 대상을 지정하며, 클래스나 메서드에 `[Preserve]` 어트리뷰트를 직접 붙이는 방법도 있습니다.

---

## 플랫폼별 런타임 제약

모든 플랫폼에서 Mono와 IL2CPP를 마음대로 고를 수 있는 것은 아닙니다. 보안 정책이나 플랫폼 아키텍처에 따라 특정 런타임 방식이 강제되거나 아예 막히기도 합니다.

---

### iOS: JIT 불가 → IL2CPP 필수

앞서 JIT 컴파일이 런타임에 새 코드를 만들어 낸다는 특성을 봤습니다. 그런데 Apple의 iOS는 보안 정책상 **실행 시점에 새 실행 코드를 메모리에 만드는 것을 허용하지 않습니다**. JIT 컴파일은 런타임에 기계어를 만들어 메모리에 올리는 과정이라, 이 정책에 막혀 동작하지 못합니다.
...
그래서 iOS 빌드는 반드시 IL2CPP를 써야 하고, Unity의 iOS 빌드 설정에서 Scripting Backend로 IL2CPP만 고를 수 있는 것도 이 때문입니다. 이 JIT 금지 정책은 Unity 게임만이 아니라 iOS의 모든 서드파티 앱 프로세스에 똑같이 적용됩니다.

예외는 WKWebView로 웹 콘텐츠를 띄울 때입니다. 이때 WebKit은 Apple이 관리하는 별도의 WebContent 프로세스에서 돌아가 그 프로세스에서는 JIT가 허용되는데, Safari가 JIT를 쓸 수 있는 것도 이 구조 덕분입니다. 반대로 JavaScriptCore를 앱 프로세스 안에 직접 임베드하면 JIT 없이 인터프리터 모드로만 동작합니다.
...
WebGL은 게임을 웹 브라우저에서 돌리기 위한 빌드 플랫폼입니다. 브라우저 안에서는 네이티브 바이너리를 직접 실행할 수 없으므로, Unity는 IL2CPP가 만든 C++ 코드를 **Emscripten** 컴파일러로 **WebAssembly(Wasm)**로 바꿉니다. Emscripten은 C/C++ 코드를 브라우저에서 실행 가능한 Wasm 바이너리로 옮겨 주는 도구입니다.
...
WebGL에서는 코드가 브라우저의 보안 샌드박스 안에서 실행됩니다. 브라우저 자체는 JavaScript나 Wasm을 JIT 컴파일하지만 이는 브라우저 엔진 내부의 일이고, 외부 코드가 제멋대로 기계어를 만들어 실행하지는 못합니다. Mono 런타임이 브라우저 안에서 JIT를 돌릴 수 없으니, IL2CPP를 통한 AOT 컴파일이 유일한 길입니다.

런타임 방식 말고도 브라우저 환경에서 오는 제약이 더 있습니다. C# 수준의 멀티스레딩(`System.Threading`)은 쓸 수 없는데, WebAssembly가 GC가 다른 스레드를 동기적으로 멈추는 메커니즘을 지원하지 않기 때문입니다. 네이티브(C/C++) 수준의 스레딩만 실험적으로 열려 있습니다.

파일 시스템 접근도 브라우저의 가상 파일 시스템으로 묶이고, `Thread.Sleep`이나 동기적 파일 I/O처럼 메인 스레드를 막는 패턴도 쓸 수 없습니다. 브라우저가 내주는 메모리에도 상한이 있어, 대규모 에셋을 한꺼번에 올리면 메모리 부족으로 크래시가 날 수 있습니다.
...
모바일 게임의 최종 빌드가 IL2CPP로 만들어진다는 것은, C# 코드가 IL과 C++를 거쳐 기계어로 실행된다는 뜻입니다. 그러니 리플렉션 제한과 코드 스트리핑, 제네릭 인스턴스화 같은 IL2CPP 환경의 제약을 염두에 두고 코드를 써야 빌드가 안정적으로 나옵니다.
...
에디터에서는 Mono로 빠르게 개발하고, 기기 빌드에서는 IL2CPP로 성능 이점과 플랫폼 호환성을 챙기는 구조입니다.

2단계에서 IL2CPP 빌드를 정기적으로 돌리는 이유는, Mono에서 멀쩡하던 코드가 IL2CPP에서 말썽을 일으킬 수 있어서입니다. 코드 스트리핑 절에서 다룬 리플렉션 접근 타입의 제거 문제나, AOT 환경에서 특정 제네릭 인스턴스화가 빠지는 문제가 대표적입니다. 예를 들어 `List<MyCustomStruct>`를 코드에서 직접 쓰지 않고 리플렉션으로만 만들면, AOT 컴파일러가 그 제네릭 인스턴스를 미리 만들어 두지 않아 런타임에 `ExecutionEngineException`이 날 수 있습니다.

AOT 환경에서는 JIT처럼 실행 도중에 새 제네릭 인스턴스를 찍어낼 수 없으므로, 빌드 시점에 쓰일 모든 제네릭 타입 조합이 코드에 명시적으로 드러나 있어야 합니다.
