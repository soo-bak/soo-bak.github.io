...
게임에서 무거운 계산을 여러 코어로 나눌 때 별도의 프로세스가 아니라 스레드를 쓰는 이유도 여기에 있습니다. 프로세스는 각자 메모리가 벽으로 막혀 있어, 데이터를 주고받으려면 한쪽 것을 복사해 건네야 합니다. 반면 한 프로세스 안의 스레드들은 같은 힙 메모리를 함께 쓰므로, 데이터를 복사하지 않고 그대로 둔 채 작업만 코어별로 나눠 맡을 수 있습니다.
...
세 방식은 서로를 대체하는 방식이 아니라, 상황에 따라 골라 쓰는 도구입니다. 코드가 어느 스레드에서 실행되는지(그래서 Unity API를 호출할 수 있는지), GC 할당이 생기는지, 어떤 작업에 적합한지가 방식마다 다릅니다. 이 절에서는 세 방식을 이 기준들로 하나씩 비교합니다.
