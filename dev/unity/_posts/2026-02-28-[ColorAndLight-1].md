...
지금까지 빛을 다룬 글은 주로 구현과 비용에서 출발했습니다. [조명과 그림자 (1) - 실시간 조명과 베이크](/dev/unity/LightingAndShadows-1/)에서는 라이팅 방식에 따른 비용 차이를 다루었고, [셰이더 최적화 (1) - 셰이더 성능의 원리](/dev/unity/ShaderOptimization-1/)에서는 셰이더 연산이 GPU에 어떤 부담을 주는지 살펴보았습니다. 이번 글에서는 그 계산이 모방하는 대상, 즉 빛과 표면 사이에서 실제로 일어나는 현상을 들여다봅니다.
...
이 시리즈는 빛의 물리적 성질에서 시작해, 게임 그래픽스가 그 현상을 어떤 수학과 셰이딩 모델로 근사하는지 차례로 따라갑니다. 첫 글에서는 빛을 전자기파로 보는 관점에서 출발해, 반사·굴절·흡수와 에너지 보존, 그리고 BRDF 개념까지 살펴봅니다.

---

## 빛이란

빛은 **전자기파(Electromagnetic Wave)**입니다. 전자기파는 **전기장(Electric Field)**과 **자기장(Magnetic Field)**이 함께 진동하며 공간을 지나가는 파동입니다.

전기장과 자기장은 모두 **전하(Electric Charge)**에서 생깁니다. 전기장은 전하가 주변 공간에 만드는 힘의 장이고, 자기장은 움직이는 전하, 즉 전류가 만드는 힘의 장입니다. 전자기파에서는 이 두 장이 서로 수직으로 진동하며 함께 퍼져 나갑니다.

전자기파는 진공에서 초속 약 3억 m, 즉 약 30만 km/s로 전파됩니다. 같은 전자기파라도 **파장(Wavelength)**이 달라지면 그 성질과 이름도 달라지며, 라디오파부터 감마선까지가 모두 파장 범위만 다를 뿐 같은 전자기파입니다.
...
망막에는 빛을 받아들이는 세 종류의 **원추세포(Cone Cell)**가 있고, 각각 S(Short), M(Medium), L(Long)으로 나뉘어 짧은 파장대, 중간 파장대, 긴 파장대에 민감하게 반응합니다. 사람이 느끼는 색은 세 원추세포가 자극되는 비율로 결정되므로, RGB는 실제 파장 분포 전체를 저장하지 않고도 인간의 색 지각을 실용적으로 근사할 수 있습니다.
...
매끄러운 표면에서 나타나는 반사가 **정반사(Specular Reflection)**입니다. 표면이 매끄러우면 미세한 법선 방향이 거의 같으므로, 들어온 빛은 표면에 수직인 **법선(Normal)**을 기준으로 대칭이 되는 방향으로 반사됩니다. 거울, 매끄러운 금속, 잔잔한 수면이 이런 반사를 보입니다.

정반사에서는 반사광이 특정 방향으로 집중되므로, 관찰자가 그 방향에 있을 때만 표면이 강하게 빛나 보입니다. 게임에서 금속이나 매끄러운 표면 위에 나타나는 밝은 점, 즉 **하이라이트(Specular Highlight)**는 이 정반사를 근사한 결과입니다.
...
**난반사(Diffuse Reflection)**는 거친 표면에서 나타나는 반사입니다. 거친 표면에는 눈에 잘 보이지 않는 미세한 요철이 있어 지점마다 법선 방향이 조금씩 다르고, 들어온 빛은 서로 다른 미세 법선을 만나 여러 방향으로 반사됩니다. 종이, 나무, 콘크리트, 피부 같은 표면이 이런 반사를 보입니다.
...
빛이 이렇게 여러 방향으로 고르게 분산되므로, 보는 방향이 달라져도 난반사 표면의 밝기는 비슷하게 유지됩니다.

이 성질을 이상화한 모델이 **Lambert 표면**입니다. Lambert 표면에서 밝기를 정하는 것은 보는 방향이 아니라 빛이 들어오는 각도, 즉 입사광과 법선이 이루는 입사각입니다. 이 밝기는 입사각의 **코사인(cos)** 값으로 표현됩니다.

입사각이 밝기를 정하는 이유는 같은 양의 빛이 닿는 표면 면적이 입사각에 따라 달라지기 때문입니다. 빛이 표면에 수직으로 들어오면 에너지가 좁은 면적에 집중되지만, 비스듬히 들어올수록 같은 에너지가 더 넓은 면적으로 퍼져 단위 면적이 받는 빛이 줄어듭니다. 그래서 입사각이 0도에 가까울수록 표면이 밝아지고, 입사각이 커질수록 밝기는 cos 값에 비례해 작아집니다.
...
빛이 방향을 바꾸는 이유는 매질마다 빛의 속도가 다르기 때문입니다. 빛의 속도가 일정하다고 할 때 그 값은 진공에서의 속도이고, 빛이 물질 속을 지날 때는 그보다 느려집니다. 같은 빛이라도 공기에서는 비교적 빠르게 진행하고, 물이나 유리에서는 더 느리게 진행합니다.
...
빛이 얼마나 꺾이는지는 두 매질에서 빛의 속도가 얼마나 다른지에 따라 정해집니다. 각 매질에서 빛이 얼마나 느려지는지를 나타내는 값이 **굴절률(Index of Refraction, IOR)**입니다.
...
빛이 두 매질의 경계에 닿으면 일부는 반사되어 되돌아 나오고, 일부는 매질 안으로 투과해 들어갑니다. 투과한 빛이 어느 방향으로 꺾이는지는 앞서 본 스넬의 법칙으로 구할 수 있습니다.

한편 들어온 빛이 반사와 투과로 얼마씩 나뉘는지는 빛이 표면에 들어오는 각도에 따라 달라집니다. 이렇게 각도에 따라 반사·투과 비율이 달라지는 현상을 **프레넬 효과(Fresnel Effect)**라고 합니다. 표면에 거의 수직으로 들어오는 빛은 대부분 안으로 투과하고 반사는 약하지만, 표면을 스치듯 비스듬히 들어올수록 반사가 강해지고 투과는 줄어듭니다.
...
빛은 표면에서 굴절되어 물체 안으로 들어갑니다. 하지만 안으로 들어간 빛이 모두 다시 밖으로 나오는 것은 아닙니다. 물질 속을 지나며 일부 에너지는 사라질 수 있습니다.
...
Albedo가 빨간 물체에 파란 빛만 비추면, 물체는 그 파란 빛을 거의 반사하지 못합니다. 그렇다고 반사할 빨간 빛이 들어오는 것도 아니어서, 물체는 어둡게 보입니다. 셰이더는 이 상호작용을 조명 색상과 Albedo를 채널별로 **곱(multiply)**해서 계산합니다.
...
이 곱셈은 흡수와 반사를 RGB 채널 수준에서 근사한 것입니다. Albedo의 채널 값이 0에 가까울수록 해당 빛은 흡수되고, 1에 가까울수록 반사됩니다. 다만 실제 빛이 늘 닿은 자리에서 흡수되거나 반사되는 것은 아닙니다.

<br>

### 서브서피스 스캐터링 (Subsurface Scattering)

물체 안으로 들어간 빛 가운데 일부는 내부 입자에 여러 번 부딪혀 방향을 바꾸며 이동하다가, 처음 들어온 지점과 다른 위치로 빠져나옵니다. 이 현상을 **서브서피스 스캐터링(Subsurface Scattering, SSS)**이라고 합니다.

피부, 대리석, 밀랍, 우유처럼 빛이 내부를 어느 정도 통과하는 재질에서 SSS가 두드러집니다. 빛이 표면 아래로 퍼졌다가 나오기 때문에, 이런 재질은 가장자리와 얇은 부분이 부드럽고 은은하게 보입니다.
...
셰이더가 내부 산란을 다루는 방식은 빛이 물질 안에서 얼마나 멀리 이동하는지에 따라 달라집니다. 나무나 돌처럼 불투명한 재질에서는 빛이 들어간 위치와 다시 나오는 위치가 거의 같다고 볼 수 있습니다. 이 경우에는 표면 한 점에서 일어나는 난반사, 예를 들어 Lambert 모델로 충분히 근사할 수 있습니다.
...
에너지 보존이 그래픽스에서 중요한 이유는 이를 지키지 않았던 과거 모델에서 드러납니다. PBR 이전에 널리 쓰이던 Phong이나 Blinn-Phong 모델은 Diffuse 성분과 Specular 성분을 따로 계산한 뒤 더하는 방식이었습니다.
...
이 제약은 러프니스(Roughness) 파라미터에서 구체적으로 나타납니다. PBR 셰이더는 표면을 수많은 **미세 면(microfacet)**의 집합으로 보고, 러프니스는 이 미세 면들의 법선 방향이 얼마나 넓게 퍼져 있는지를 나타냅니다.

러프니스를 높이면 미세 면 법선이 넓게 퍼지고, 반사된 빛도 여러 방향으로 분산됩니다. 하이라이트는 넓어지지만 가장 밝은 부분의 밝기는 낮아집니다. 하이라이트 면적이 넓어졌는데 밝기까지 그대로라면, 반사 에너지 총량이 입사 에너지를 넘기 때문입니다.
...
앞에서 반사, 굴절, 흡수가 에너지 보존이라는 제약 안에서 함께 일어난다는 점을 살펴봤습니다. 게임에서 다루는 불투명한 표면이라면 눈에 들어오는 빛은 대부분 반사된 빛이고, 표면이 어떻게 보이는지도 결국 이 반사광이 정합니다.

그런데 반사광은 빛의 양만으로 정해지지 않습니다. 같은 빛이 들어와도 거울은 빛을 한 방향으로 모으고 분필은 사방으로 흩뿌리듯, 표면에 따라 빛이 나가는 방향과 세기가 크게 달라집니다. 그래서 셰이더는 반사광을 하나의 값이 아니라 방향에 따른 분포로 계산해야 합니다.
...
BRDF가 물리적으로 타당하려면 몇 가지 조건을 만족해야 합니다. 그중 대표적인 것이 **헬름홀츠 상호성**과 **에너지 보존**입니다.
...
이 조건들이 지켜지지 않으면 반사 결과가 물리적으로 어긋납니다. 에너지 보존이 깨지면 표면이 받은 빛보다 더 밝아 보이고, 상호성이 깨지면 시점이 바뀔 때 반사가 일관되지 않습니다.
...
지금까지 본 BRDF는 표면에서 일어나는 **반사**만 기술합니다. 그런데 빛은 반사만 하는 것이 아니라 표면 안으로 투과되기도 하므로, 투과는 BRDF의 범위 밖에 있습니다.

유리나 물처럼 빛이 표면을 통과해 반대편으로 나가는 **투과**는 **BTDF(Bidirectional Transmittance Distribution Function)**가 기술합니다. 반사와 투과를 함께 아우르는 함수는 **BSDF(Bidirectional Scattering Distribution Function)**입니다.

다만 게임에 등장하는 표면은 대부분 불투명해서, 반사만 기술하는 BRDF로도 많은 재질을 표현할 수 있습니다. 투과가 필요한 재질조차 BTDF를 엄밀하게 계산하기보다 알파 블렌딩이나 화면 공간 굴절 같은 근사로 처리하는 경우가 많습니다. 따라서 실시간 렌더링에서는 여전히 BRDF가 표면 반사 계산의 중심입니다.
...
이 현상들은 따로 떨어져 있지 않습니다. 반사·굴절·흡수는 에너지 보존이라는 제약 안에서 함께 일어나고, BRDF는 그중 표면 반사를 방향에 따른 분포로 나타냅니다.

<br>

셰이딩 계산은 이런 물리 원리를 바탕으로 하지만, 그 결과를 화면의 픽셀로 옮기려면 **색공간(Color Space)**이라는 디지털 표현 체계를 거쳐야 합니다.

다음 글 [색과 빛 (2) - 색 표현과 색공간](/dev/unity/ColorAndLight-2/)에서는 RGB로 색을 표현하는 방식부터 감마 보정, 선형 워크플로우, HDR까지 차례로 살펴봅니다.
